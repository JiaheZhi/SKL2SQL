...
with open('/root/volume/SKL2SQL/experiments/results/usa_accident_rf_deep5_frequency_compration2_thread1.txt', 'r') as file:
    for line in file:
        for fusion in fusions:
            match = re.search(r'(\w+)_' + fusion + '_2' + r'_(\d+) 的执行时间为: (\d+\.\d+)s', line)
            if match:
...
# 保存图像到 JPEG 文件
plt.savefig('/root/volume/SKL2SQL/experiments/results/usa_accident_rf_deep5_frequency_compration2_thread1.png')
